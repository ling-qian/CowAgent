...
from config import available_setting, conf, get_root, pconf, plugin_config, write_plugin_config
...
class CloudClient(LinkAIClient):
    ...
    def on_config(self, config: dict):
        ...
        if self._set_channel_credentials(
            local_config, current_channel_type, config.get("app_id"), config.get("app_secret")
        ):
            need_restart_channel = True
    ...
    def _handle_channel_create(self, channel_type: str, data: dict):
        ...
        if skip_restart and channel_type in ("weixin", "wx"):
            ...
            if login_status != "logged_in":
                skip_restart = False
                logger.info(
                    f"[CloudClient] Channel '{channel_type}' not logged in (status={login_status}), forcing restart"
                )
        if skip_restart:
            logger.info(
    ...
    def _remove_weixin_credentials():
        ...
        cred_path = os.path.expanduser(conf().get("weixin_credentials_path", "~/.weixin_cow_credentials.json"))
        try:
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _set_channel_credentials(local_config: dict, channel_type: str, app_id, app_secret) -> bool:
        """
    ...
    def _restart_channel(self, new_channel_type: str):
        ...
        if self.channel_mgr:
            ...
            threading.Thread(
                target=self._do_restart_channel, args=(self.channel_mgr, new_channel_type), daemon=True
            ).start()
        else:
