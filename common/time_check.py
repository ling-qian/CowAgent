...
def time_checker(f):
    def _time_checker(self, *args, **kwargs):
        ...
        if chat_time_module:
            ...
            # 结束时间小于开始时间，跨天了
            if (chat_stop_time < chat_start_time and (chat_start_time <= now_time or now_time <= chat_stop_time)) or (
                chat_start_time < chat_stop_time and chat_start_time <= now_time <= chat_stop_time
            ):
                f(self, *args, **kwargs)
            else:
