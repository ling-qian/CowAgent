...
def run():
    ...
    try:
        ...
        # 多租户：saas_mode=True 时初始化 PostgreSQL 数据库 + 管理 API
            ...
            try:
                ...
                if database_url:
                    ...
                    try:
                        ...
                        logger.debug(f"[App] Prometheus metrics not available: {e}")
                    # 在子线程中启动 Flask 管理 API
                    saas_port = int(conf().get("saas_api_port", 8081))
                    threading.Thread(
                        target=lambda: saas_app.run(
                            host="0.0.0.0", port=saas_port, debug=False
                        ),
                        daemon=True,
                    ).start()
                    logger.info(f"[App] SaaS management API started on port {saas_port}")
                else:
                    logger.warning("[App] saas_mode=True but database_url not configured")
            except Exception as e:
                logger.warning(f"[App] SaaS database init failed: {e}")
