...
available_setting = {
    ...
    # SaaS 多租户配置
    "saas_mode": False,  # 是否启用 SaaS 多租户模式。False 时行为与原版完全一致
    "database_url": "",  # PostgreSQL 连接串，saas_mode=True 时必填。格式: postgresql://user:pass@host:5432/dbname
    "saas_api_port": 8081,  # SaaS 管理 API 端口（Flask），saas_mode=True 时生效
}
...
def conf_tenant(tenant_id=None):
    """获取当前租户的配置（saas_mode=True 时生效）。

    优先级：租户级覆盖 > 全局配置 > available_setting 默认值。
    saas_mode=False 时等价于 conf()，无额外开销。

    Args:
        tenant_id: 显式指定租户ID，不传则从 TenantContext 获取
    """
    if not config.get("saas_mode", False):
        return config
    from saas.config_loader import conf_tenant as _conf_tenant
    return _conf_tenant(tenant_id)
